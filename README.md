...
- **Name notes** (Settings, off by default): each question first asks you to name the **lower** then the **upper** note by tapping one of seven letter buttons (C–B, with 도–시 underneath) before the Quality/Number buttons appear. Instant judging with retry on wrong (no reveal); the named note plays when you get it right.
